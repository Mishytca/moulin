...
def handle_utility_call(argv: List[str]) -> int:
    ...
    # Reorder layers if the current order differs from the YAML manifest
    need_reorder = used_build_layers_abs != yaml_layers_abs
    ...
    if need_reorder:
        ...
        # Synchronize only missing/extra layers
        to_remove_abs = [
    ...
    remove_cmd = (
        "bitbake-layers remove-layer " + " ".join(to_remove_abs)
        if to_remove_abs else ""
    )

    add_cmd = (
        "bitbake-layers add-layer " + " ".join(to_add_abs)
        if to_add_abs else ""
    )
